...
export const scalarToLiteralMapping: {
  ...
  "std::int64": { extraTypes: ["string"], type: "number" },
  ...
  "std::str": { literalKind: "typeof", type: "string" },
  "std::uuid": { type: "string" },
  "std::json": { type: "unknown" },
  "std::bool": { literalKind: "typeof", type: "boolean" },
  "std::bigint": { literalKind: "typeof", type: "bigint" },
  "std::bytes": { literalKind: "instanceof", type: "Uint8Array" },
  "std::datetime": {
    extraTypes: ["string"],
...
export function getLiteralToScalarMapping(version: Version) {
  ...
  for (const [scalarType, { literalKind, type }] of Object.entries(scalarToLiteralMapping)) {
    if (literalKind) {
      ...
      literalToScalarMapping[type] = { literalKind, type: scalarType };
    }
  }
...
export function toTSScalarType(
  ...
  switch (type.kind) {
    case "scalar": {
      ...
      return [
        (literalType.startsWith("edgedb.") ? opts.edgedbDatatypePrefix : "") + literalType
      ];
    }
    ...
    case "tuple": {
      ...
      if (type.tuple_elements[0].name && Number.isNaN(parseInt(type.tuple_elements[0].name, 10))) {
        // a named tuple
        const res = [];
...
export function makeValidIdent({ id, name, skipKeywordCheck }: { id: string; name: string; skipKeywordCheck?: boolean; }) {
...
export function frag(strings: TemplateStringsArray, ...exprs: (CodeFragment | CodeFragment[])[]) {
...
export function joinFrags(frags: (CodeFragment | CodeFragment[])[], sep: string) {
...
export async function writeDirWithTarget(dir: DirBuilder, target: Target, params: { outputDir: string; written?: Set<string> }) {
...
export type Target = "cjs" | "deno" | "esm" | "mts" | "ts";
